...
"""
Script to download all files from the urls specified in the excel report
...
# Use this only when reference to SSL is a problem while generating distributables
# Create "certifi" folder with ".pem" file under the folder where exe is present			
...
class XmlExtractor():
    ...
    def get_report_details_gui(self):
        ...
        self.folder_name_button = tk.Button(self.root, text="Select Folder", command=self.select_folder, width=17,
                                            pady=5)
    ...
    def iterate_report(self):
        ...
        for row in range(2, max_row + 1):
            if (sheet.cell(row=row, column=8).value == "FinishedSuccessfully"):
                pii = sheet.cell(row=row, column=1).value.split(":")[1]
                ...
                try:
                    self.download_xml(url, pii + "_" + supplier + "_" + asset + ".xml")
                except Exception as e:
                    logging.exception(
                        "Error extracting xml for pii:{0} from {1} for asset type {2}".format(pii, supplier, asset))
                    continue
